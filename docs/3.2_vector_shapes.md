...
Users can click **Load** button ![button](imgs/button_load.png) or **File->Shapes->Load** menu item to open an ArcGIS shapefile (\*.shp) as a **Shapes**. Creating a new **Shapes** can be done from menu **File->Shapes->New** and the pop-up dialog as shown below. Users can choose one shapes type from **Point**![icon_shapes_point](imgs/icon_shapes_point.png), **Line**![icon_shapes_line](imgs/icon_shapes_line.png) and **Polygon**![icon_shapes_polygon](imgs/icon_shapes_polygon.png). The relationship between a ArcGIS shapefile (\*.shp) and a same named dBase file (\*.dbf) is also shown.
