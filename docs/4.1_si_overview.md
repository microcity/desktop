...
Following is an example of a embedded function description.
```lua
AddParameter (Module, parent_node, "Number",  "ID" [, default = 0])
```
To clarify the parameter type in a embedded function, we use name and quotation mark to avoid ambiguities. In this case, the *Module* parameter is an *Module* object, the *"Number"* parameter is a string, and the *default* parameter is a number. The contents in a pair of '\[' and ']' are optional. The character '|' among parameters means "or".
